...
function iniciarSesion() {
  ...
  let clienteEncontrado = buscarClientePorUsuarioYContrasenia(clientesUsers, login, passLogin);
  if (clienteEncontrado) {
    ingresoExitoso = true;
    esVete = false;
    console.log('Bienvenido(a), ' + clienteEncontrado.nomUsu + ' ' + clienteEncontrado.apeUsu + '!');
    let cantMascotas = parseInt(prompt('Ingrese la cantidad de mascotas que desea cargar: '));
    cargarMascotas(clienteEncontrado, cantMascotas);
  } else {
    // Buscar el usuario en la lista de veterinarios
    let veteEncontrado = buscarUsuario(usuariosVete, login, passLogin);
    if (veteEncontrado) {
      ingresoExitoso = true;
      esVete = true;
      console.log('Bienvenido(a), ' + veteEncontrado.nomVete + '!');
      let cantTurnos = parseInt(prompt('Ingrese la cantidad de turnos que desea cargar: '));
      cargarTurnos(veteEncontrado, cantTurnos);
    } else {
